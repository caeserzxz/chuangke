...
function openItem(param) {	
	...
	if (typeof(data_str[2]) != 'undefined'){
		$('#workspace').attr('src','/index.php?m=' + data_str[2] + '&c=' + data_str[0] + '&a=' + data_str[1]);
		 $.cookie('workspaceParam', data_str[0] + '|' + data_str[1]+ '|' + data_str[2], { expires: 1 ,path:"/"});
	}else{
    	$('#workspace').attr('src','/index.php?m=systemadmin&c=' + data_str[1] + '&a=' + data_str[0]);
