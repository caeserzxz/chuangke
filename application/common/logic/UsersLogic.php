...
class UsersLogic extends Model
{
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $map['password'] = '';
            $map['openid'] = $openid;
            $map['nickname'] = filternickname($data['nickname']);
            $map['reg_time'] = time();
            $map['oauth'] = $oauth;
            $map['head_pic'] = $data['head_pic'];
            $map['sex'] = $data['sex'] === null ? 0 :  $data['sex'];
            if (session('first_leader')) $map['first_leader'] = session('first_leader'); // 推荐人id
            if ($_GET['first_leader']) $map['first_leader'] = $_GET['first_leader']; // 微信授权登录返回时 get 带着参数的
            // 如果找到他老爸还要找他爷爷他祖父等
            if ($map['first_leader']) {
            ...
            //trace(M('OauthUsers')->getlastsql(),'debug');
    ...
    public function reg($username,$password,$password2,$push_id = 0,$invite=array(),$nickname="",$head_pic=""){
        ...
        if($is_validated != 1)
            return array('status'=>-1,'msg'=>'请输入手机号码','result'=>'');
        ...
        {
            return array('status'=>-1,'msg'=>'推荐码错误','result'=>'');
        }
    ...
    public function get_goods_collect($user_id){
        ...
        $result = M('goods_collect')->alias('c')
            ->field('c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.is_on_sale,g.store_count,g.cat_id,g.is_virtual,g.original_img,g.give_integral')
            ->join('goods g','g.goods_id = c.goods_id','INNER')
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        if($is_update && ($user['password'] != '' && encrypt($old_password) != $user['password']))
            return array('status'=>-1,'msg'=>'请重新输入原来密码','result'=>'');
    ...
    /*修改用户积分或者账户*/
    public function setAccountOrPoints($user_id,$type,$num,$other_info = [])
    {
        ...
        $log_data = array_merge($other_info,$log);

        $rs2 =M('account_log')->data($log_data)->save();
        ...
        return ['status'=>-2,'msg'=>'系统操作失败'];


    }

    /*绑定上下级关系*/
    public function bindLeader($user_id,$parent_id)
    {
        $first_leader = M('users')->where("user_id = {$parent_id}")->find();
        $map['first_leader'] = $parent_id;
        $map['second_leader'] = $first_leader['first_leader'];
        $map['third_leader'] = $first_leader['second_leader'];
        //他上线分销的下线人数要加1
        M('users')->where(array('user_id' => $map['first_leader']))->setInc('underling_number');
        ...
        //$user = Users::get($user);
        Users::where('user_id',$user_id)->save($map);

        return ['code'=>1,'msg'=>'绑定成功','url'=>U('user/index')];   
    }
     ...
     * @return [array]          
     */
    public function layer_number($user_id){
        $user=M('users')
            ->alias('u1')
            ->field("count(distinct u1.user_id) first_lower,count(distinct u2.user_id) second_lower,count(distinct u3.user_id) third_lower")
            ->join('users u2', 'u1.user_id = u2.first_leader','LEFT')
            ->join('users u3', 'u2.user_id = u3.first_leader','LEFT')
     ...
     * @return [int] [已购人数]
     */
    public function purchase_number($user_id){
        $first_ids=M('users')->where('first_leader',$user_id)->column('user_id');
        $second_ids=$first_ids ? M('users')->where('first_leader','in',$first_ids)->column('user_id') : [];
        $third_ids=$second_ids ? M('users')->where('first_leader','in',$second_ids)->column('user_id') : [];
        $user_ids=array_merge((array)$first_ids,(array)$second_ids,(array)$third_ids);
        if(empty($user_ids)) return 0;
        $count = M('order')->where('user_id','in',$user_ids)->where('order_status','in',['2','4'])->count('distinct user_id');
        return $count;
    }
