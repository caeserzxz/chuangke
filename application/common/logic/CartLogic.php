...
class CartLogic extends Model
{
    ...
    public function buyNow(){
        ...
        if (!$this->goodsBuyNum || $this->goodsBuyNum <= 0) {
            return array('status' => 0, 'msg' => '购买数量不能够为0', 'result' => '');
        }
...
//        $cartCheckAfterList = $cartList;
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getApiCartList($selected = 0){
        $cart = new Cart();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
            $cartWhere['user_id'] = $this->user_id;
        ...
        //->field('goods_name,spec_key_name,member_goods_price,goods_price,selected')
        //字段筛选
        foreach($cartCheckAfterList as $key => $value)
        {
            $value->visible(['id','goods_id','goods_name','spec_key_name','member_goods_price','goods_price','selected','goods_num','market_price','goods'=>['original_img']]);
            $value['goods']['original_img'] = api_img_url($value['goods']['original_img']);
        }
     ...
     * @return array
     */
    public function changeNum($cart_id, $goods_num,$select = ''){
        $Cart = new Cart();
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        foreach ($cart as $key => $val) {
            trace('id:'.$val['id'].':是否选中:'.$val['selected'],'debug');
    ...
    public function clean(){
        $cartWhere['user_id'] = $this->user_id;
        $delete = Db::name('cart')->where($cartWhere)->delete();
        return $delete;
    }
}