...
class DistributLogic extends Model{
    ...
    public function rebateLog($order){
        ...
        $pattern = tpCache('distribut.pattern'); //分佣模式0按商品设置的1按订单比例
        $rate    = tpCache('distribut.order_rate'); //订单设置的比例
        ...
        $user2 = Users::get($user['first_leader']);
        $user3 = Users::get($user2['first_leader']);

        Db::startTrans();

        try{
            $this->userDistributDo($user['first_leader'],1,$order);
            $this->userDistributDo($user2['first_leader'],2,$order);
            $this->userDistributDo($user3['first_leader'],3,$order);
            Db::commit();
        }catch(\Exception $e)
        {
            Db::rollback();
