...
class Cart extends MobileBase {
    ...
    public function cart2(){
        ...
        if(empty($address)){
            header("Location: " . U('Mobile/User/add_address', ['source' => 'cart2', 'goods_id' => $goods_id, 'goods_num' => $goods_num, 'item_id' => $item_id, 'action' => $action]));
            exit;
        }else{
            $region_ids = "{$address['province']},{$address['city']},{$address['district']}";
            $region = M('region')->where('id', 'in', $region_ids)->column('id,name');
            $address['region'] = $region[$address['province']] . $region[$address['city']] . $region[$address['district']];
