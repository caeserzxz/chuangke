 ...
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = '',$type=1,$status=1){
    /* 插入帐户变动记录 */
    if($user_money > 0){
        $type = 2;
    }else{
...
/*
 *获取所有下级 组成一个数组
 *@param leaders string 所有下一级id
 *@param all_arr array 所有下级
 */
function all_leader_arr($leaders = '',$all_arr = [],$key=999)
{
    if(empty($leaders)){
        return $all_arr;
    }

    $leader_arr = Db('users')->where(['first_leader'=>['In',$leaders]])->column('user_id');

    if(empty($leader_arr)){
        return $all_arr;
    }
    $leader_str = implode( ',',$leader_arr);

    $num = count($leader_arr);

    $new_arr[] = $num;
    $all_arr = array_merge($all_arr,$new_arr);
    
    if($key != 999){
        end($all_arr);
        $first_key = key($all_arr);
    ...
    return all_leader_arr($leader_str,$all_arr,$key);
...
/**
 * 获取第N层下级用户
 * @param $user_id_arr array 用户id
 * @param $n int 第N层用户
 * @author lhk
 */
function sub_user($user_id_arr,$n = 1){
    if($n >= 1){
        $user_ids = implode(',',$user_id_arr);
        $data = Db::name('users')->where(['first_leader'=>['In',$user_ids],'level'=>['gt',1]])->column('user_id');
        if($data){
            return sub_user($data,$n-1);
        }else{
            return array();
        }
    }else{
 ...
 * @author lhk
 */
function sub_user1($user_id_arr,$n = 1){
    if($n >= 1){
        $user_ids = implode(',',$user_id_arr);
        $data = Db::name('users')->where(['first_leader'=>['In',$user_ids],'level'=>['gt',1]])->column('user_id');
        if($data){
            $datas = array_values(array_unique(array_merge($user_id_arr,$data)));
            return sub_user1($datas,$n-1);
        }else{
            return sub_user1($user_id_arr,$n-1);
        }
    }else{
        return $user_id_arr;
    }
