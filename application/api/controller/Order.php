...
class Order extends Base
{
    ...
    public function refund_order()
    {
        ...
        $order['reason']   =M('reason')->where(['type'=>1])->column(['reason']);
    ...
    public function return_goods_detail()
    {
        ...
        $order_goods['shop_info_address'] = $tpshop_config['shop_info_address_detail'];
        $order_goods['shop_info_phone']   = $tpshop_config['shop_info_phone'];

        $order_goods['addretu'] = '(周一至周五) 08:00-19:00';
        $order_goods['reason']  = M('reason')->where(['type'=>2])->column(['reason']);
