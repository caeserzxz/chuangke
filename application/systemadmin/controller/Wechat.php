...
class Wechat extends Base
{
    ...
    public function add(){
        ...
        if(IS_POST){
            ...
            if($row){
                $this->success('添加成功',U('Wechat/setting',array('id'=>$row)));
            }else{
                $this->error('操作失败');
            }
            exit;
    ...
    public function setting()
    {
        ...
        if(empty($wechat)){
            return $this->error('请先在公众号配置添加公众号，才能进行微信菜单管理', U('Wechat/index'));
        }
        if(IS_POST){
    ...
    public function menu(){
        ...
        if(IS_POST){
            ...
            $this->success('操作成功,进入发布步骤',U('Wechat/pub_menu'));
            exit;
        }
        //获取最大ID
    ...
    public function pub_menu(){
        ...
        if(!$access_token){
            $this->error('获取access_token失败',U('Wechat/menu')); //  http://www.tpshop.com/index.php/systemadmin/Wechat/menu
...
//        exit($post_str);
    ...
    public function text(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行文本回复管理', U('Wechat/index'));
        }
    ...
    public function add_text(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加文本回复', U('Wechat/index'));
        }
        if(IS_POST){
            $edit = I('get.edit');
            ...
            $row ? $this->success("添加成功",U('Wechat/text')) : $this->error("添加失败",U('Wechat/text'));
            exit;
        }
    ...
    public function img(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行图文回复管理', U('Wechat/index'));
        }
    ...
    public function add_img(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加图文回复', U('Wechat/index'));
        }
        if(IS_POST){
            ...
            $row ? $this->success("添加成功",U('Wechat/img')) : $this->error("添加失败",U('Wechat/img'));
            exit;
        }
    ...
    public function news(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行多图文管理', U('Wechat/index'));
        }
    ...
    public function add_news(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加多图文', U('Wechat/index'));
        }
        if(IS_POST){
            ...
            if($arr)
                array_pop($arr);
            if(count($arr) <= 1){
                $this->error("单图文请到图文回复设置",U('Wechat/news'));
                exit;
            }
            $add['keyword'] =  I('post.keyword');
            $add['token'] =  $wechat['token'];
            ...
            //添加模式
                ...
                $add['type'] = 'NEWS';
                $row = M('wx_keyword')->add($add);
            $row ? $this->success("添加成功",U('Wechat/news')) : $this->error("添加失败",U('Wechat/news'));
            exit;
        }
        return $this->fetch();
    }
