...
class Distribut extends Base
{
    ...
    public function set()
    {
        //header("Location:" . U('Systems/index', array('inc_type' => 'distribut')));
        header("Location:" . U('Systems/index', array('inc_type' => 'reward_set')));
        exit;
    }
    ...
    public function tree()
    {
        ...
        I('user_id') && $where = "user_id =" . I('user_id');
        $count = Db::name('users')->count();// 查询满足要求的总记录数
        $Page = $pager = new Page($count, 100);// 实例化分页类 传入总记录数和每页显示的记录数
        $list = Db::name('users')->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $show = $Page->show();
    ...
    public function yesurl(){
        ...
        if($rebate['status'] != 2)
            $this->error('不能修改分成记录');
        $date=tpCache('distribut.date')*24*60*60;
        if(!$rebate['confirm'] || time()< $rebate['confirm'] + $date)
