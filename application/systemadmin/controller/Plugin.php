...
//        $row = $model->where($condition)->save($update);
    ...
    public function setting(){
        ...
        $condition['code'] = I('get.code');
        $model = M('plugin');
        if(($condition["code"] == "unionpay")){ header("Content-type: text/html; charset=utf-8");exit("功能正在开发中。。。"); }
        if($condition["type"] == "login"  && $condition["code"] == "weixin"){ header("Content-type: text/html; charset=utf-8");exit("功能正在开发中。。。"); }
        ...
        if(IS_POST){
            ...
            if(I('get.edit') == 1){
                ...
                if($row){
                    ...
                    M('area_region')->insertAll($add2)&&exit($this->success("添加成功",U('Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                }
