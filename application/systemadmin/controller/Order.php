...
class Order extends Base {
    ...
    public function activity_status_list(){
        $activity_type = I('activity_type');
        if($activity_type){
            if($activity_type == 3){
                $result = M('prom_goods')->field('title,id')->select();
            }elseif($activity_type == 2){
                $result = M('group_buy')->field('title,id')->select();
            }else{
                $result = '';
            }
            $this->ajaxReturn(['status' => 1 , 'msg' => 'ok!' , 'result' => $result]);
        }else{
    ...
    public function ajaxindex(){
        ...
        $order_prom_type =  ($keyType && $keyType == 'order_prom_type') ? $keywords : I('order_prom_type','','trim');
        $order_prom_type ? $condition['order_prom_type'] = trim($order_prom_type) : false;//活动类型
        $order_sn ? $condition['order_sn'] = trim($order_sn) : false;//订单

        $order_prom_id = ($keyType && $keyType == 'order_prom_id') ? $keywords : I('order_prom_id') ;
        $order_prom_id ? $condition['order_prom_id'] = trim($order_prom_id) : false;//活动ID
...
exit("功能正在开发中。。。");
    ...
    public function return_info()
    {
        ...
        if($return_goods['imgs']){
            $_img =substr($return_goods['imgs'], 0 , -1);
           $return_goods['imgs'] = explode(',', $_img);
        }
    ...
    public function refund_balance(){
		...
		$this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Order/return_list")]);
    ...
    public function add_return_goods()
   {
            ...
            {
                $this->error('已经提交过退货申请!',U('Order/return_list'));
                exit;
            }
            $order = M('order')->where("order_id = $order_id")->find();
            ...
            $this->success('申请成功,现在去处理退货',U('Order/return_list'));
            exit;
    }
        ...
        if($action && $order_id){
            if($action !=='pay'){
        	 ...
        	 if($res !== false && $a !== false){
                 if ($action == 'remove') {
                     exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => U('order/index')))));
                 }
        	 	exit(json_encode(array('status' => 1,'msg' => '操作成功')));
        	 }else{
                 if ($action == 'remove') {
                     exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => U('order/index')))));
                 }
    ...
    public function export_order()
    {
        ...
        I('pay_status') != '' ? $condition['pay_status'] = I('pay_status') : false;//支付状态
        I('pay_code') != '' ? $condition['pay_code'] = I('pay_code') : false;//支付方式
        I('shipping_status') != '' ? $condition['shipping_status'] =['in',I('shipping_status')] : false;//发货状态
        I('user_id') ? $condition['user_id'] = trim(I('user_id')) : false;

        if(!empty(I('order_ids'))){
            $condition['order_id'] =  array('in',$order_ids);
        }
        $orderList = Db::name('order')->field("*,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time")->where($condition)->order('order_id')->select();
        ...
        if(IS_POST)
        {
           ...
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
            ...
            if($order_id)
            {
                ...
                $this->success('添加商品成功',U("Order/detail",array('order_id'=>$order_insert_id)));
                exit();
            }
            else{
                $this->error('添加失败');
