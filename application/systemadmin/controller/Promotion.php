...
use app\systemadmin\model\FlashSale;
use app\systemadmin\model\Goods;
use app\systemadmin\model\GoodsActivity;
use app\systemadmin\model\GroupBuy;
...
//        }
    ...
    public function get_goods()
    {
        ...
        foreach ($goodsList as $key => $val) {
            $_t['goods_name'] = $val['goods_name'];
            $_t['shop_price'] = $val['shop_price'];
            $_t['store_count'] = $val['store_count'];
            $_t['prom_type'] = $val['prom_type'];
            $_t['prom_id'] = $val['prom_id'];
            $_t['spec_goods_price'] = $val['spec_goods_price'];
            $_list[] = $_t;
        }
        //修改代码
        $show = $Page->show();
        $this->assign('page', $show);
        $this->assign('goodsList', $_list);
        return $this->fetch();
    }
    ...
    {
        $this->assign("URL_upload", U('Ueditor/imageUp', array('savepath' => 'promotion')));
        $this->assign("URL_fileUp", U('Ueditor/fileUp', array('savepath' => 'promotion')));
        $this->assign("URL_scrawlUp", U('Ueditor/scrawlUp', array('savepath' => 'promotion')));
        $this->assign("URL_getRemoteImage", U('Ueditor/getRemoteImage', array('savepath' => 'promotion')));
        $this->assign("URL_imageManager", U('Ueditor/imageManager', array('savepath' => 'promotion')));
        $this->assign("URL_imageUp", U('Ueditor/imageUp', array('savepath' => 'promotion')));
        $this->assign("URL_getMovie", U('Ueditor/getMovie', array('savepath' => 'promotion')));
        $this->assign("URL_Home", "");
    }

    public function downGodos(){
         $id = I('id');
         $table = I('table');
        $field = I('field') ? I('field') :'id' ;
        if(!in_array($table,['prom_goods','group_buy'])){
            exit(json_encode(['status'=>-1,'msg'=>'系统参数有误~table']));
        }

        $prom = M($table)->where($field,$id)->find();

        if(!$prom)
            exit(json_encode(['status'=>-1,'msg'=>'操作有误,id不存在']));
