...
use app\systemadmin\logic\SearchWordLogic;
use think\AjaxPage;
use think\Loader;
use think\Page;
use think\Db;
...
class Goods extends Base {
    ...
    public function addEditCategory(){
            ...
            if(I('is_ajax') == 1)
            {
                ...
                if(!$validate->batch()->check(input('post.')))
                {
                    ...
                    $id=I('id')?:0;
                    $same_cate = M('GoodsCategory')->where(['parent_id'=>$par_id , 'mobile_name'=>$GoodsCategory['mobile_name'], 'id'=>['neq',$id]])->find();
                    if($same_cate){
                        $return_arr = array(
                            'status' => 0,
                            'msg' => '同级已有相同分类存在',
    ...
    public function delGoodsCategory(){
        ...
        $this->ajaxReturn(['status' => 1,'msg' =>'操作成功','url'=>U('Goods/categoryList')]);
    }
    ...
    {
        $GoodsLogic = new GoodsLogic();
        $Goods = new \app\systemadmin\model\Goods();
        $goods_id = I('goods_id');
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            $return_url = $is_distribut > 0 ? U('Distribut/goods_list') : U('Goods/goodsList');
            $data = input('post.');
    ...
    public function delGoods()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("goods/goodsList")]);
    }
    ...
    public function delGoodsType()
    {
        ...
        $count > 0 && $this->error('该类型下有商品规格不得删除!',U('Goods/goodsTypeList'));
        // 判断 商品属性
        $count = M("GoodsAttribute")->where("type_id = {$id}")->count("1");
        $count > 0 && $this->error('该类型下有商品属性不得删除!',U('Goods/goodsTypeList'));
        // 删除分类
    ...
    public function delGoodsAttribute()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!",'url'=>U('Goods/goodsAttributeList')]);
    }
    ...
    public function delGoodsSpec()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!!!",'url'=>U('Goods/specList')]);
    }
